...
DROP TABLE if exists $ANCDATABASE.patients_migated_in_anc;

CREATE TABLE $ANCDATABASE.patients_migated_in_anc as 
...
union
SELECT ANC_patient_id FROM $ANCDATABASE.anc_remaining_diff_gender
union
SELECT ANC_patient_id FROM $ANCDATABASE.anc_art_patients_with_voided_art_identifier;


DROP TABLE if exists $ANCDATABASE.anc_patients_with_duplicate_males_art;

CREATE TABLE $ANCDATABASE.anc_patients_with_duplicate_males_art as 
SELECT pi.patient_id AS ANC_patient_id, (SELECT @max_patient_id + pi.patient_id) AS ART_patient_id, pn.family_name, pn.given_name, pi.identifier, p.gender, p.birthdate FROM patient_identifier pi
...
where pi.identifier_type = 3 and pi.voided = 0 and pi.patient_id not in (SELECT ANC_patient_id FROM $ANCDATABASE.patients_migated_in_anc);
...
from $ANCDATABASE.person p 
 inner join $ANCDATABASE.anc_patients_with_duplicate_males_art pp ON pp.ANC_patient_id = p.person_id and p.voided = 0
...
from $ANCDATABASE.patient p 
	inner join $ANCDATABASE.anc_patients_with_duplicate_males_art pp ON pp.ANC_patient_id = p.patient_id and p.voided = 0
	left join $ANCDATABASE.user_bak c on c.ANC_user_id = p.creator
	left join $ANCDATABASE.user_bak u on u.ANC_user_id = p.changed_by;
...
from $ANCDATABASE.encounter e 
	inner join $ANCDATABASE.anc_patients_with_duplicate_males_art pp ON pp.ANC_patient_id = e.patient_id and e.voided = 0
