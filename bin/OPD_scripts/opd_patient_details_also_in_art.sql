...
DROP TABLE IF EXISTS openmrs_area18_opd.user_backup;

create table openmrs_area18_opd.user_backup as
...
FROM openmrs_area18_opd.users where username not in (select u.username from openmrs_area18_opd.users u inner join openmrs_area18_art.users a where u.username = a.username)
UNION
...
FROM openmrs_area18_opd.users u INNER JOIN openmrs_area18_art.users a WHERE u.username = a.username;

/*  Create an opd users table to be migrated */
DROP TABLE IF EXISTS openmrs_area18_opd.user_details_to_be_migrated;

create table openmrs_area18_opd.user_details_to_be_migrated as
SELECT * FROM openmrs_area18_opd.user_backup b 
WHERE b.OPD_user_id NOT IN (SELECT u.user_id FROM openmrs_area18_opd.users u INNER JOIN openmrs_area18_art.users a WHERE u.username = a.username);
...
from openmrs_area18_opd.person p inner join openmrs_area18_opd.user_details_to_be_migrated u on u.person_id = p.person_id;
...
from openmrs_area18_opd.person_name f 
inner join openmrs_area18_opd.user_details_to_be_migrated u on u.person_id = f.person_id;

/* Insert OPD users */
INSERT INTO openmrs_area18_art.users (user_id,  system_id,  username,  password,  salt,  secret_question,  secret_answer,  creator,  date_created,  changed_by,  date_changed,  person_id,  retired,  retired_by,  date_retired,  retire_reason,  uuid,  authentication_token)
SELECT ART_user_id,  system_id,  username,  password,  salt,  secret_question,  secret_answer, creator,  date_created,  changed_by,  date_changed, person_id,  retired, retired_by,  date_retired,  retire_reason,  (SELECT UUID()),  authentication_token FROM openmrs_area18_opd.user_details_to_be_migrated; 

/* Create OPD patient details table */
drop table if exists openmrs_area18_opd.opd_patient_details;

create table openmrs_area18_opd.opd_patient_details
select p.person_id, p.gender, p.birthdate, p.death_date, pn.family_name, pn.given_name, pi.identifier, pi.identifier_type from patient_identifier pi  inner join person p on p.person_id = pi.patient_id  inner join person_name pn on pn.person_id = p.person_id where pi.voided = 0 and pi.identifier_type = 3 order by pi.patient_id;

/* Create ART patient details table */
drop table if exists openmrs_area18_art.art_patient_details;

create table openmrs_area18_art.art_patient_details
select p.person_id, p.gender, p.birthdate, p.death_date, pn.family_name, pn.given_name, pi.identifier, pi.identifier_type
from openmrs_area18_art.patient_identifier pi  inner join openmrs_area18_art.person p on p.person_id = pi.patient_id  inner join openmrs_area18_art.person_name pn on pn.person_id = p.person_id where pi.voided = 0 and pi.identifier_type IN (3, 2) and pi.patient_id not in (select person_id from openmrs_area18_art.users) order by pi.patient_id;

/* Create patients that are not in ART */
DROP TABLE IF EXISTS openmrs_area18_opd.opd_patient_details_in_art;

CREATE TABLE openmrs_area18_opd.opd_patient_details_in_art AS
...
FROM openmrs_area18_opd.opd_patient_details p INNER JOIN openmrs_area18_art.art_patient_details a ON a.identifier = p.identifier WHERE (a.given_name = p.given_name AND a.family_name = p.family_name) AND (p.gender = a.gender AND p.birthdate = a.birthdate);

/* creating encounters table back-up */
DROP TABLE IF EXISTS openmrs_area18_opd.encounter_back;

CREATE TABLE openmrs_area18_opd.encounter_back AS
...
FROM openmrs_area18_opd.opd_patient_details_in_art a INNER JOIN openmrs_area18_opd.encounter e ON e.patient_id = a.OPD_patient_id WHERE e.voided = 0  group by e.patient_id, e.encounter_id, e.encounter_type, DATE(e.encounter_datetime);
...
select e.encounter_id, e.encounter_type, e.patient_id, ART_user_id, e.location_id, e.form_id, e.encounter_datetime, u.ART_user_id, e.date_created, e.voided, e.voided_by, e.date_voided, e.void_reason, (select uuid()), e.changed_by, e.date_changed, 14 from openmrs_area18_opd.encounter_back e inner join openmrs_area18_opd.user_backup u ON e.creator = u.OPD_user_id group by e.patient_id, e.encounter_id, e.encounter_type, DATE(e.encounter_datetime);

/* creating orders back-up */
drop table if exists openmrs_area18_opd.orders_bak;

create table openmrs_area18_opd.orders_bak as
SELECT order_id as OPD_order_id, (SELECT @max_order_id + o.order_id) as ART_order_id, order_type_id, concept_id, orderer, (SELECT @max_encounter_id + o.encounter_id) as encounter_id, instructions, start_date, auto_expire_date, discontinued, discontinued_date, discontinued_by, discontinued_reason, (SELECT @max_user_id + creator) as creator, date_created, voided, voided_by, date_voided, void_reason, e.ART_patient_id AS patient_id, accession_number, (SELECT @max_obs_id + o.obs_id) as obs_id, uuid, discontinued_reason_non_coded FROM openmrs_area18_opd.orders o inner join openmrs_area18_opd.opd_patient_details_in_art e on e.OPD_patient_id = o.patient_id WHERE o.voided = 0;
...
SELECT o.ART_order_id, o.order_type_id, o.concept_id, o.orderer, o.encounter_id, o.instructions, o.start_date, o.auto_expire_date, o.discontinued,  o.discontinued_date, o.discontinued_by, o.discontinued_reason, u.ART_user_id, o.date_created, o.voided,  o.voided_by,  o.date_voided,  o.void_reason, o.patient_id, o.accession_number, o.obs_id,  (SELECT UUID()),  o.discontinued_reason_non_coded FROM openmrs_area18_opd.orders_bak o inner join openmrs_area18_opd.user_backup u ON o.creator = u.OPD_user_id
...
SELECT o.ART_order_id, drug_inventory_id, dose, equivalent_daily_dose, units, frequency, prn, complex, quantity FROM openmrs_area18_opd.drug_order d inner join openmrs_area18_opd.orders_bak o on o.OPD_order_id = d.order_id GROUP BY o.OPD_order_id;

/* creating obs back-up */
drop table if exists openmrs_area18_opd.obs_bak;

create table openmrs_area18_opd.obs_bak as
...
FROM openmrs_area18_opd.obs o inner join openmrs_area18_opd.opd_patient_details_in_art e on e.OPD_patient_id = o.person_id inner join openmrs_area18_opd.user_backup u on u.OPD_user_id = o.creator WHERE o.voided = 0 group by o.obs_id;
...
SELECT ART_obs_id, person_id,  concept_id, encounter_id,  order_id,  obs_datetime,  location_id,  obs_group_id,  accession_number,  value_group_id,  value_boolean,  value_coded,  value_coded_name_id,  value_drug,  value_datetime,  value_numeric, value_modifier,  value_text,  date_started,  date_stopped,  comments,  creator,  date_created,  voided, voided_by,  date_voided,  void_reason,  value_complex,  (SELECT UUID()) FROM openmrs_area18_opd.obs_bak;

/* creating patient_program back-up */
drop table if exists openmrs_area18_opd.patient_program_backup;

create table openmrs_area18_opd.patient_program_backup as
...
FROM openmrs_area18_opd.patient_program p INNER JOIN openmrs_area18_opd.opd_patient_details_in_art e on e.OPD_patient_id = p.patient_id INNER JOIN openmrs_area18_opd.user_backup u on u.OPD_user_id = p.creator GROUP BY p.patient_program_id;
...
select ART_patient_program_id,  patient_id,  program_id,  date_enrolled,  date_completed,  creator,  date_created, changed_by,  changed_by,  voided, voided_by,  date_voided,  void_reason,  (SELECT UUID()),  location_id from openmrs_area18_opd.patient_program_backup f order by patient_id;
...
SELECT p.ART_patient_program_id, f.state, f.start_date, f.end_date, p.creator, f.date_created, f.changed_by, f.date_changed, f.voided, f.voided_by, f.date_voided, f.void_reason, (SELECT UUID()) FROM  openmrs_area18_opd.patient_state f INNER JOIN openmrs_area18_opd.patient_program_backup p on p.OPD_patient_program_id = f.patient_program_id ORDER BY patient_program_id;
