...
/* Finding patients that have more than 1 identifier */
DROP TABLE IF EXISTS openmrs_area18_opd.opd_patients_with_multiple_identifier;

CREATE TABLE openmrs_area18_opd.opd_patients_with_multiple_identifier AS
select patient_id, count(*) as num from openmrs_area18_opd.patient_identifier where voided = 0 and identifier_type in (3) 
and identifier not in (select identifier from openmrs_area18_art.patient_identifier where voided = 0) group by patient_id having num > 1;

/* dropping and creating person_back_up */
DROP TABLE IF EXISTS openmrs_area18_opd.opd_remaining_patients;

CREATE TABLE openmrs_area18_opd.opd_remaining_patients AS
select patient_id AS OPD_person_id,  (SELECT @max_patient_id + patient_id) AS ART_person_id from openmrs_area18_opd.patient_identifier where voided = 0 
and identifier_type in (3) 
and identifier not in (select a.identifier from openmrs_area18_art.patient_identifier a where a.voided = 0) 
and patient_id not in (select o.patient_id from openmrs_area18_opd.opd_patients_with_multiple_identifier o);
...
FROM openmrs_area18_opd.person p 
 INNER JOIN openmrs_area18_opd.opd_remaining_patients pp ON pp.OPD_person_id = p.person_id AND p.voided = 0
...
FROM openmrs_area18_opd.patient p 
	INNER JOIN openmrs_area18_opd.opd_remaining_patients pp ON pp.OPD_person_id = p.patient_id AND p.voided = 0
...
FROM openmrs_area18_opd.encounter e 
	INNER JOIN openmrs_area18_opd.opd_remaining_patients pp ON pp.OPD_person_id = e.patient_id and e.voided = 0
