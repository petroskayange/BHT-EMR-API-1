...
echo "Msigrating OPD patients that are not in ART"
...
DROP TABLE IF EXISTS $OPDDATABASE.OPD_only_patients_details;

CREATE TABLE $OPDDATABASE.OPD_only_patients_details AS
SELECT pi.patient_id AS OPD_person_id, (SELECT @max_patient_id + pi.patient_id) AS ART_person_id 
FROM $OPDDATABASE.patient_identifier pi WHERE pi.patient_id not in (SELECT o.opd_patient_id FROM $OPDDATABASE.opd_patient_details_in_art o) AND pi.identifier_type = 3 
AND pi.identifier not in (SELECT p.identifier FROM $DATABASE.patient_identifier p WHERE p.voided = 0 AND p.identifier_type in (2, 3));
...
FROM $OPDDATABASE.person p 
 INNER JOIN $OPDDATABASE.OPD_only_patients_details pp ON pp.OPD_person_id = p.person_id AND p.voided = 0
...
FROM $OPDDATABASE.patient p 
  INNER JOIN $OPDDATABASE.OPD_only_patients_details pp ON pp.OPD_person_id = p.patient_id AND p.voided = 0
...
FROM $OPDDATABASE.encounter e 
  INNER JOIN $OPDDATABASE.OPD_only_patients_details pp ON pp.OPD_person_id = e.patient_id and e.voided = 0
...
echo "Finished migrating OPD patients that are not in ART"
