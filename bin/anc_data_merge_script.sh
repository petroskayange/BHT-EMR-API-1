...
SELECT ART_user_id,  system_id,  username,  password,  salt,  secret_question,  secret_answer, creator,  date_created,  changed_by,  date_changed, person_id,  retired, retired_by,  date_retired,  retire_reason,  (SELECT UUID()),  authentication_token FROM $ANCDATABASE.user_bak; 
...
SELECT order_id,  order_type_id,  concept_id,  orderer, encounter_id,  instructions, start_date,  auto_expire_date,  discontinued,  discontinued_date,  discontinued_by,  discontinued_reason,  creator,  date_created,  voided,  voided_by,  date_voided,  void_reason, patient_id, accession_number, obs_id,  (SELECT UUID()),  discontinued_reason_non_coded FROM $ANCDATABASE.orders_bak;
...
echo "Running another script to merge patients with different gender"
./bin/anc_migrating_remaining_patients.sh development

echo "Running another script to merge patients with voided identifier"
./bin/anc_migrating_patients_with_voided_ART_identifier.sh development

echo "Running another script to insert the remaining patient"
