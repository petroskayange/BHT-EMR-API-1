...
describe MaternityService::WorkflowEngine do
  let(:epoch) { Time.now }
  let(:evr_program) { program 'MATERNITY PROGRAM' }
  let(:patient) { create :patient }
  let(:engine) do
    MaternityService::WorkflowEngine.new program: evr_program,
                                   patient: patient,
                                   date: epoch
  end
  ...
  describe :next_encounter do
    it 'returns REGISTRATION for patient not in EVR programme' do
      evr_program
      encounter_type = engine.next_encounter
      expect(encounter_type.name.upcase).to eq('REGISTRATION')
    end

    it 'returns REGISTRATION for new EVR patient' do
      enroll_patient patient
      encounter_type = engine.next_encounter
      expect(encounter_type.name.upcase).to eq('REGISTRATION')
    end

    it 'returns ADMIT PATIENT after PATIENT REGISTRATION' do
      register_patient patient
      encounter_type = engine.next_encounter
      expect(encounter_type.name.upcase).to eq('ADMIT PATIENT')
    end

    it 'returns SOCIAL HISTORY after initial state' do
      encounter_type = engine.next_encounter
      expect(encounter_type.name.upcase).to eq('SOCIAL HISTORY')
    end
  ...
  def register_patient(patient, date = nil)
    date ||= Time.now
    enroll_patient patient
    create :encounter, type: encounter_type('REGISTRATION'),
                       patient: patient,
                       date_created: date
  end
